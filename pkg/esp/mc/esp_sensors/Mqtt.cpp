...
const char *mqtt_broker = "monorail.proxy.rlwy.net"; // EMQX broker endpoint
  ...
  if (WS_MQTT_status == MQTT_CONNECTED) {
    ...
    if (MSG_length == 4) {
      if (MSG_payload[0] == 32 && MSG_payload[3] == 0) {
        ...
        AllSensorsData initData = mqttSensors.getAllSensorsData(true);
        json = prepareData(initData);
